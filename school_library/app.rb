...
class App
  ...
  def self.initialize
    @should_save_data = false # Initialize the flag as false
  end
  ...
  def self.run
    initialize # Initialize the flag
    choice = display_menu
    choice = handle_choice(choice) while choice != 7
    save_data_to_files if @should_save_data # Save data if the flag is set to true
  ...
  def self.set_should_save_data(value)
